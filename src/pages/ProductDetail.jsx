...
export default function ProductDetail() {
    ...
    const images = useMemo(() => {
        ...
        if (imgs.length === 0) {
            const fallback = [];
            if (product?.main_image) fallback.push({id: "main", url: product.main_image, is_main: true});
            if (product?.hover_image) fallback.push({id: "hover", url: product.hover_image, is_main: false});
            return fallback;
        }
    ...
    if (notFound) {
        return (<div className="container pay-result">
    ...
    const selectedPrimaryOption = selectedSizeOpt ?? selectedFormatOpt ?? selectedCapacityOpt ?? null;
    const isCustomizable = Boolean(product?.is_customizable);

    // Navigates to customizer
    ...
    return (<div className="pd">
        <section className="pd-gallery" aria-label="Galerie produit">
            {images.length > 0 ? (images.map((img, index) => (<button
        ...
        <aside className="pd-panel">
            <div className="pd-header">
                {product?.supplier?.name && (<p className="pd-brand">GAUTHIER FITNESS</p>)}
            ...
            {product?.variants?.length > 0 && (<div className="pd-block">
                ...
                <div className="pd-swatches">
                    {product.variants.map((v) => {
                        const label = v.variant_value_label || v.flavor_label || v.color_label || v.slug;
            ...
            {sizeOptions.length > 0 && (<div className="pd-block">
                <div className="pd-block-head">
                    <h4>Taille</h4>
                    <button
                        type="button"
            ...
            {formatOptions.length > 0 && (<div className="pd-block">
                <div className="pd-block-head">
                    <h4>Format</h4>
                </div>
            ...
            {capacityOptions.length > 0 && (<div className="pd-block">
                <div className="pd-block-head">
                    <h4>Capacité</h4>
                </div>
            ...
            <div className="pd-cta">
                <button
                    ...
                    onClick={async () => {
                        try {
                            await addItem({
                                productId: product.id, optionId: selectedPrimaryOption?.id ?? null, quantity: 1,
                            });
                            openCart();
                        } catch (e) {
            ...
            {isCustomizable && (<div className="pd-cta" style={{marginTop: 12}}>
                <button
                    type="button"
                    className="pd-add"
                    disabled={!canAddToCart}
