...
export default function AddressesPage() {
    ...
    const [form, setForm] = useState({
        address: user?.address || "", zip: user?.zip || "", city: user?.city || "",
    });
    ...
    async function submit(e) {
        ...
        try {
            ...
            if (validation && typeof validation === "object") {
                ...
                setError(err?.response?.data?.message || "Impossible de mettre à jour l'adresse. Réessaie dans un instant.");
            }
        } finally {
            setLoading(false);
        }
    ...
    return (<div className="pay-result">
        ...
        {!editing ? (<div className="pay-result-box mt-lg text-center">
            <div style={{fontWeight: 700}}>Par défaut</div>

            {hasAddress ? (<div className="ck-muted mt-sm">
                {user.address && <div>{user.address}</div>}
                <div>{[user.zip, user.city].filter(Boolean).join(" ")}</div>
                <div>France</div>
            </div>) : (<div className="ck-muted mt-sm">
        ...
        </div>) : (<form onSubmit={submit} className="pay-result-box mt-lg" style={{maxWidth: 360}}>
