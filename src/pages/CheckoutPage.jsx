...
// Loads previously saved checkout form from localStorage
...
export default function CheckoutPage() {
    ...
    const shippingCost = useMemo(() => estimateShippingCost(form.shippingMethod, subtotal), [form.shippingMethod, subtotal]);
    const total = useMemo(() => subtotal + shippingCost, [subtotal, shippingCost]);

    function update(key, value) {
        setForm((prev) => ({...prev, [key]: value}));

        if (clientSecret && ["address", "zip", "city", "country", "firstname", "lastname", "shippingMethod"].includes(key)) {
    ...
    async function createIntent() {
        ...
        try {
            const payload = {
                email: form.email, shipping_method: form.shippingMethod, shipping: {
                ...
                <section className="checkout-left">
                    ...
                    >
                        ...
                        <div className="ck-block">
                            ...
                            <div className="ck-radio-card">
                                {SHIPPING_METHODS.map((m) => {
                                    ...
                                    return (<label key={m.value} className="ck-radio">
                                        <input
                                            type="radio"
                                            name="shippingMethod"
                                            value={m.value}
                        ...
                        <div className="ck-block">
                            ...
                            </button>) : (elementsOptions && (
                                <Elements stripe={stripePromise} options={elementsOptions}>
                                    <CheckoutPayment email={form.email} clientSecret={clientSecret}/>
                                </Elements>))}

                            {items.length === 0 && (<p className="ck-muted" style={{marginTop: 10}}>
                                Ton panier est vide.
                            </p>)}
                        </div>
                    </form>
                ...
                <aside className="checkout-right" aria-label="récapitulatif commande">
                    <div className="ck-summary">
                        <div className="ck-summary-items">
                            {items.map((it) => (<div key={it.key} className="ck-summary-item">
                                ...
                                <div className="ck-summary-info">
                                    ...
                                    {it.isCustomized && (<div className="ck-summary-meta" style={{marginTop: 4}}>
                                        Produit personnalisé
                                    </div>)}

                                    {it.customization?.player_name?.value && (<div className="ck-summary-meta">
                                        Nom : {it.customization.player_name.value}
                                    </div>)}

                                    {it.customization?.player_number?.value && (<div className="ck-summary-meta">
