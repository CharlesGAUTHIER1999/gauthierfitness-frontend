...
// Constants
const TEXT_COLORS = [{label: "Noir", value: "#111111"}, {label: "Blanc", value: "#ffffff"}, {
    label: "Rouge", value: "#c62828"
}, {label: "Bleu", value: "#1565c0"},];

const PRODUCT_COLOR_SWATCHES_3D = [{label: "Bleu fitness", value: "#1d4ed8"}, {
...
const PRESET_LOGOS = [{label: "Aucun logo", value: ""}, {label: "UBB", value: "/logos/ubb.png"}, {
...
}, {label: "Badge noir", value: "/logos/badge-black.jpg"}, {label: "Badge blanc", value: "/logos/badge-white.jpg"},];

const PATTERN_OPTIONS = [{id: "motif1", label: "Motif 1", thumb: "/motifs/motif1.png"}, {
    id: "motif2", label: "Motif 2", thumb: "/motifs/motif2.png"
}, {id: "motif3", label: "Motif 3", thumb: "/motifs/motif3.png"},];

const GRADIENT_OPTIONS = [{id: "degrade1", label: "Dégradé 1", thumb: "/degrades/degrade1.jpg"}, {
    id: "degrade2", label: "Dégradé 2", thumb: "/degrades/degrade2.jpg"
}, {id: "degrade3", label: "Dégradé 3", thumb: "/degrades/degrade3.jpg"},];

const TABS = [{id: "style", label: "Style"}, {id: "texte", label: "Texte"}, {id: "medias", label: "Médias"},];

// Internal sub-components
function VisualGallery({title, enabled, selectedId, options, onToggle, onSelect}) {
    const [open, setOpen] = useState(false);

    return (<div className="pc-accordion">
        ...
        {open && (<div className="pc-style-gallery">
...
function UploadButton({label, accept, disabled, loading, onChange, children}) {
    ...
    return (<div className="pc-upload-wrap">
        <button
            type="button"
...
function StyleTab({
    ...
    return (<div className="pc-tab-body">
        {/* Application zone */}
        ...
        {/* Product color */}
        {mode === "3d" ? (<div className="pc-field-group">
            <p className="pc-field-label">Couleur du vêtement</p>
            <div className="pc-swatches-row">
                {PRODUCT_COLOR_SWATCHES_3D.map((swatch) => (<button
                    key={swatch.value}
                    type="button"
                    title={swatch.label}
        ...
        </div>) : (variants.length > 0 && (<div className="pc-field-group">
            <p className="pc-field-label">Couleur / variante</p>
            <div className="pc-variants-grid">
                {variants.map((variant) => {
                    const label = variant.variant_value_label || variant.flavor_label || variant.color_label || variant.slug;
...
// "Text" tab
function TexteTab({
                      configuration, onTextColorChange, onPlayerNameChange, onPlayerNumberChange, onAddTextLayer,
                  }) {
    return (<div className="pc-tab-body">
        {/* Text color */}
        <div className="pc-field-group">
            ...
            <div className="pc-color-grid">
                {TEXT_COLORS.map((color) => (<button
                    key={color.value}
                    type="button"
                    className={`pc-color-chip ${configuration?.text_style?.color === color.value ? "is-active" : ""}`}
...
function MediasTab({
    ...
    const isUploadedLogo = Boolean(configuration?.logo?.src) && !PRESET_LOGOS.some((l) => l.value === configuration?.logo?.src);

    const currentViewImageLayers = Array.isArray(configuration?.image_layers) ? configuration.image_layers.filter((layer) => (layer?.view || "front") === currentView) : [];
    ...
    return (<div className="pc-tab-body">
        ...
        <div className="pc-field-group">
            ...
            {!configuration?.logo?.enabled && (<p className="pc-medias-hint">
            ...
            {configuration?.logo?.enabled && (<>
                ...
                >
                    {PRESET_LOGOS.map((logo) => (<option key={logo.value} value={logo.value}>
                        {logo.label}
                    </option>))}
                </select>
                ...
                >
                    {uploadLogoError && (<p className="pc-upload-error">{uploadLogoError}</p>)}
                    {isUploadedLogo && (<p className="pc-upload-ok">Logo personnalisé importé</p>)}
                </UploadButton>

                {Boolean(configuration?.logo?.src) && (<button
                    type="button"
        ...
        <div className="pc-field-group">
            <p className="pc-field-label">
                Image libre
                {currentViewImageLayers.length > 0 && (<span className="pc-field-count">
            ...
            >
                {uploadImageError && (<p className="pc-upload-error">{uploadImageError}</p>)}
            </UploadButton>

            {currentViewImageLayers.length > 0 && (<div className="pc-image-list">
                {currentViewImageLayers.map((layer, index) => (<div
        ...
        {aiEnabled && (<AiDesignForm
            onGenerate={onGenerateAiDesign}
            loading={aiLoading}
            error={aiError}
        />)}

        {aiLocked && (<div className="pc-field-group">
...
export default function CustomizationPanel({
    ...
    const currentStyle = useMemo(() => configuration?.style?.[currentView] || {
    ...
    return (<aside className="pc-sidebar">
        ...
        <div className="pc-panel-header">
            ...
            {product?.name && (<p className="pc-panel-subtitle">{product.name}</p>)}
        </div>
