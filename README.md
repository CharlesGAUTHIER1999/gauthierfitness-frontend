...
| State global | Zustand 5 (auth, cart) + Context React                      |
...
│   ├── SizeGuideDrawer.jsx · UserIcon.jsx
│   └── StaticPage.jsx
...
- **State** : Zustand pour l'état partagé (auth, cart). Context React pour les valeurs locales à un sous-arbre. Pas de
  Redux.
...
Pas encore de tests unitaires JS, prévus en V1.5. Les tests E2E Playwright sont gérés depuis le repo **infra** (
`infra/e2e/`) et ciblent le staging déployé.

CI : `lint-build` (ESLint + build Vite) → `build image GHCR` → `dispatch infra`. Cf. [
